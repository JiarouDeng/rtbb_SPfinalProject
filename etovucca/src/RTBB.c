...
char *url_decode(const char *src)
{
   ...
   char *dest = malloc(len + 1); // Allocate memory for the decoded string
   char *ptr = dest;             // Pointer to write to the decoded string
   ...
   while (src < end)
   {
      if (*src == '%')
      {
         ...
         if (src + 2 < end)
         {
            ...
            sscanf(src + 1, "%2x", &value); // Read the next two characters
            *ptr++ = (char)value;           // Convert to char
            src += 3;                       // Move past %XX
         }
         else
         {
            // Invalid encoding, just copy the remaining part
            *ptr++ = *src++;
         }
      }
      else if (*src == '+')
   ...
   *ptr = '\0'; // Null-terminate the decoded string

   return dest; // Return the decoded string
}
