...
try:
    ...
    if len(form) != 0:
        ...
        unqiue_candidate_id = str(
            elections[date_info.group()]["offices"][int(id_info.group(1))][
                "candidates"
        ...
        subprocess.check_output(
            [
                PATH_TO_MACHINE,
                "vote",
                form.getvalue("voterId"),
                str(convert_date_to_id(form.getvalue("election")[: id_info.span()[0]])),
                unique_office_id,
                unqiue_candidate_id,
            ]
        )
        ...
        print("<ul>")
        print("<li>Election Date: {}</li>".format(date_info.group()))
        print(
            "<li>Office: {}</li>".format(
                elections[date_info.group()]["offices"][int(id_info.group(1))]["name"]
            )
        )
        print(
            "<li>Candidate: {}</li>".format(
                elections[date_info.group()]["offices"][int(id_info.group(1))][
                    "candidates"
                ][int(id_info.group(2))]["name"]
            )
        )
        print("</ul>")
    else:
            ...
            if elections[date]["status"] == "open":
                for oid in range(0, len(elections[date]["offices"])):
                    ...
                    for cid in range(0, len(office["candidates"])):
                        ...
                        print(
                            '<option value="{}_{}_{}">{}</option>'.format(
                                date, oid, cid, candidate["name"]
                            )
                        )
                    print("</optgroup>")
        print("</select>")
