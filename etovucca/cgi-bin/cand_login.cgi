...
try:
    ...
    json_candidate = subprocess.check_output([PATH_TO_MACHINE, "get-candidate"]).decode('utf-8')
    candidates = json.loads(json_candidate)	
    
    if len(form) != 0:
        ids = form.getvalue('candidate').split('_')
        unqiue_candidate_id = str(candidates[int(ids[0])]['id'])
        subprocess.check_output([PATH_TO_MACHINE, 'get-candidate', form.getvalue('id'), unqiue_candidate_id])
