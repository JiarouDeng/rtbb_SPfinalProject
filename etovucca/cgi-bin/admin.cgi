...
def render_elections(elections, status):
    ...
    for date in elections:
        if elections[date]["status"] == status:
            ...
            if status == "open":
                elections_category.append(
                    '<li>Election ID {}: {} <a href="admin.cgi?action=closed&id={}">Close</a>'.format(
                        eid, date, eid
                    )
                )
            elif status == "closed":
                elections_category.append(
                    '<li>Election ID {}: {} <a href="admin.cgi?action=open&id={}">Open</a> <a href="admin.cgi?action=published&id={}">Publish</a>'.format(
                        eid, date, eid, eid
                    )
                )
            else:
                elections_category.append(
                    '<li>Election ID {}: {} <a href="admin.cgi?action=deleted&id={}">Delete</a>'.format(
                        eid, date, eid
                    )
                )
            ...
            for oid in range(0, len(elections[date]["offices"])):
                ...
                elections_category.append(
                    "<li>Office ID {}: {}".format(office["id"], office["name"])
                )
                elections_category.append("<ul>")
                ...
                for cid in range(0, len(office["candidates"])):
                    ...
                    if status == "published":
                        elections_category.append(
                            "<li>Candidate ID {}: {} ({} votes)".format(
                                candidate["id"], candidate["name"], candidate["votes"]
                            )
                        )
                    else:
                        elections_category.append(
                            "<li>Candidate ID {}: {}".format(
...
try:
        ...
        if "action" in form:
            if form.getvalue("action") == "open":
                subprocess.check_output(
                    [PATH_TO_MACHINE, "open-election", form.getvalue("id")]
                )
            if form.getvalue("action") == "closed":
                subprocess.check_output(
                    [PATH_TO_MACHINE, "close-election", form.getvalue("id")]
                )
            if form.getvalue("action") == "published":
                subprocess.check_output(
                    [PATH_TO_MACHINE, "publish-election", form.getvalue("id")]
                )
            if form.getvalue("action") == "deleted":
                subprocess.check_output(
                    [PATH_TO_MACHINE, "delete-election", form.getvalue("id")]
                )
            print(
                '<b>Successfully set election {} to "{}".</b>'.format(
                    form.getvalue("id"), form.getvalue("action")
                )
            )
        elif "addElection" in form:
            subprocess.check_output(
                [PATH_TO_MACHINE, "add-election", form.getvalue("addElection")]
            )
            print(
                "<b>Successfully added election {}</b>".format(
                    form.getvalue("addElection")
                )
            )
            ...
            print(
                "<b>Successfully added candidate {} to office {}</b>".format(
                    form.getvalue("addCandidate"), form.getvalue("office")
                )
            )
        elif "addZip" in form:
            subprocess.check_output(
                [
                    ...
                    "add-zip",
                    form.getvalue("office"),
                    form.getvalue("addZip"),
                ]
            )
            print(
                "<b>Successfully added ZIP {} to office {}</b>".format(
                    form.getvalue("addZip"), form.getvalue("office")
                )
            )

    json_elections = subprocess.check_output([PATH_TO_MACHINE, "get-elections"]).decode(
        "utf-8"
    )
    ...
    json_voters = subprocess.check_output([PATH_TO_MACHINE, "get-voters"]).decode(
        "utf-8"
    )
    ...
    for voter in voters:
        print(
            "<li>{} ({}): {}, {}".format(
                voter["name"], voter["dob"], voter["county"], voter["zip"]
            )
        )
    print("</ul>")
